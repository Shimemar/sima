...
主な用途は、DevKit 上で動作する AI/画像処理アプリの検証と、ホストPC との連携確認です。実験時に使う設定ファイル、起動手順メモ、GUI/CLI 実装を集約しています。
...
このフォルダは、実験用・検証用のワークスペースとして設計されています。実行環境やモデル配置パス、IP アドレス、ホスト情報が README や設定ファイルに記載されている場合があるため、利用時は環境に合わせて調整してください。
