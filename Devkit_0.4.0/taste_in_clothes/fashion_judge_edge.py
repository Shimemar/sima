#!/usr/bin/env python3
"""
コーデ辛口ジャッジ - エッジ版 (Modalix DevKit)

入力: 全身が写った静止画ファイル
処理: YOLO26で人物検出・クロップ -> Gemma 4 (E2B, VisionLanguageModel経由) で
      毒舌ギャル系ファッション判定
出力: ターミナルにスコア・毒舌コメントを表示

使い方:
    python fashion_judge_edge.py --image path/to/photo.jpg

Modalix DevKit上でpyneatが使える環境でのみ動作する(`dk`経由で実行するか、
DevKit上のPython環境で直接実行する)。

--- 実装メモ ---
detect_person_bbox() / judge_fashion() は、既存の隣接アプリと同じ pyneat API を
そのまま踏襲している。VLMモデルのディレクトリ名は `ls /media/nvme/llima/models`
をDevKit上で実行して実在を確認したパス(gemma-4-E2B-it-GPTQ-a16w4)を使う:

  * 人物検出: /workspace/vlm_ngen_demo (webcam版YOLO26人物検出+VLM) と同じ
    yolo26m-det-bf16-mla_tess-b1.tar.gz (COCO class 0 = person, BoxDecodeType.YoloV26,
    NormalizePreset.COCO_YOLO) を使い、Model.run() で1枚画像に対する同期推論を行う
    (Model.build([seed], ...) はこのデバイスでタイムアウトするため使わない --
    静止画の一発推論なので Model.run() で十分)。
  * VLM判定: /workspace/vlm_ngen_demo/vlm_commenter.py と同じ
    pyneat.genai.VisionLanguageModel + GenerationRequest を使う。

API検証元: core/include/model/Model.h, core/docs/develop-apps/development-workflow/model.mdx,
core/include/genai/VisionLanguageModel.h, core/include/genai/GenAITypes.h,
/workspace/vlm_ngen_demo/main.py, /workspace/vlm_ngen_demo/vlm_commenter.py
"""
...
JUDGE_PROMPT = """あなたは「毒舌ギャル系ファッション審査員」です。渡された全身コーディネート写真を見て、服・色使い・アイテムのバランス・小物・トレンド感といったファッション要素だけを対象に、辛口・毒舌全開で評価してください。

【厳守ルール】
- 体型、顔立ち、外見そのものへの言及や否定は絶対にしないこと。あくまで「服・スタイリング」への評価に限定する。
- ギャル語(マジ、ウケる、それな、あーね、やば、しか勝たん、エモい 等)を交えた口調にすること。
- 毒舌ではあるが人格否定にはせず、最後にちょっとだけ愛のあるフォローを入れること。
- 100点満点でスコアをつけること。
...
{
  "score": 0から100の整数,
  "verdict": "全体評価を一言でズバッと言うキャッチコピー(20〜30文字程度)",
  "roasts": ["色使い・バランス・小物・トレンド感などについての具体的な毒舌コメントを3〜4個の配列で"],
  "compliment": "毒舌の中にもある本音の褒めポイントを1文で",
  "closing": "締めの一言。次に期待してる的なノリで1文"
}"""
...
def preload_yolo_model(width: int, height: int) -> None:
    ...
    最悪パイプライン自体がタイムアウトし続ける壊れた状態になった(2026-09-08、
    実機テストで確認・この場当たり的な対策は撤回済み)。

    実際に安定して動くのは、CLI版(fashion_judge_edge.py単発実行)と同じ
    「その画像の実サイズでビルドする」方式のみ。サーバーではリクエストごとに
    画像サイズが変わりうるので、detect_person_bbox() 側で「サイズが変わったら
    再ビルド」するようにした(_yolo_model_size を参照)。この関数はサーバー
    起動時に典型的なサイズで一度ビルドしておき、最初のリクエストの待ち時間を
    減らすためだけのもの — 実際に来た画像のサイズと違えば
    detect_person_bbox() が透過的に再ビルドする。
    """
...
def judge_fashion(cropped_image: np.ndarray) -> JudgeResult:
    """
    クロップ済み画像(uint8 HWC RGB)を渡してファッション判定JSONを取得する。

    pyneat.genai.VisionLanguageModel + GenerationRequest を使い、Gemma 4 (E2B) に
    JUDGE_PROMPT と画像を渡して1回のリクエストで判定JSONテキストを得る。
    """
    global _vlm_model
    import pyneat
...
def print_result(result: JudgeResult) -> None:
    ...
    print()
    print(f"{Ansi.PINK}辛口ポイント:{Ansi.RESET}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="コーデ辛口ジャッジ (エッジ版)")
    ...
    try:
        ...
        return _fail(
            ...
            "でのみ動作する(`dk ./fashion_judge_edge.py --image ...` 等で実行)。",
            args.json,
        )
    ...
    try:
        ...
        print("VLM判定中(Gemma 4 via llima)...", file=sys.stderr)
