#!/usr/bin/env python3
"""
コーデべた褒めジャッジ - サーバー版・べた褒めバージョン (Modalix DevKit上で常駐するHTTPサーバー)

`fashion_judge_server.py`(毒舌ギャル系)の派生版。HTTPサーバーとしての
仕組み(エンドポイント・グローバルロック・ウォームアップ手順)は完全に同じで、
検出/判定ロジックのimport元だけを `fashion_judge_edge` から
`fashion_judge_edge_rave`(べた褒め版のJUDGE_PROMPT)に差し替えている。
...
使い方(DevKit上、`dk`経由):
    ...
    dk ./fashion_judge_server_rave.py --port 8766

既定ポートは毒舌版(8765)と衝突しないよう8766にしてある -- 同じDevKit上で
毒舌版・べた褒め版を同時に起動して使い分けることもできる(YOLO26/VLMの
モデルはプロセスごとに別々にロードされるため、両方同時起動時はメモリ・MLA
の負荷が2倍になる点に注意)。
...
テストで不安定だったため採用していない -- fashion_judge_edge_rave.
...
import fashion_judge_edge_rave as fje
...
class JudgeRequestHandler(BaseHTTPRequestHandler):
    server_version = "FashionJudgeServerRave/1.0"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="コーデべた褒めジャッジ (サーバー版・rave)")
    parser.add_argument("--host", default="0.0.0.0", help="待受アドレス(既定: 0.0.0.0)")
    parser.add_argument("--port", type=int, default=8766, help="待受ポート(既定: 8766)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            ...
            "でのみ動作する(`dk ./fashion_judge_server_rave.py --port ...` 等で実行)。",
            file=sys.stderr,
        )
        return 1
    ...
    print("両モデルのロード完了(べた褒めモード)。", file=sys.stderr)
