#!/usr/bin/env python3
"""
DevKit直結HDMI出力・べた褒めバージョン - USBカメラ(v4l2)の映像をModalix DevKit自身の
HDMI出力にそのまま表示する。マウスクリックで撮影し、コーデべた褒めジャッジの結果も
すべてHDMI上のオーバーレイとして表示する(Tkinter等の別ウィンドウは使わない --
webcam_hostgui.pyのHOSTPC GUI版と同じ機能を、DevKit本体のHDMI出力+マウスだけで
完結させたもの)。

`devkit_direct_hdmi.py`(毒舌版)の派生版。判定パイプライン・GStreamer
パイプライン・マウス入力・オーバーレイ描画の仕組みはすべて同じで、接続先の
判定サーバーだけを毒舌版(`fashion_judge_server.py`、既定ポート8765)から
べた褒め版(`fashion_judge_server_rave.py`、既定ポート8766)に差し替えて
ある(`DEFAULT_SERVER_URL`とブランド文字の既定値のみの違い)。
...
5. 画面右端・上下中央に出すブランド文字「コーデべた褒めジャッジ」は日本語なので
...
8. 撮影した写真をコーデ判定サーバー(`fashion_judge_server_rave.py`)に渡すのは
   HTTPのまま(`hostpc_gui.py`と同じプロトコル)だが、このスクリプトは
   DevKit本体上で動く(サーバーも同じDevKit上)ので既定の宛先は
   `http://127.0.0.1:8766`(べた褒め版サーバーの既定ポート)にしてある。
...
Run (DevKit上、`dk`経由):
    ...
    dk ./devkit_direct_hdmi_rave.py
    dk ./devkit_direct_hdmi_rave.py --camera-device /dev/video0 --width 1280 --height 720 --fps 30
    dk ./devkit_direct_hdmi_rave.py --no-overlay          # ブランド文字オーバーレイなし
    dk ./devkit_direct_hdmi_rave.py --overlay-text "べつのテキスト"
    dk ./devkit_direct_hdmi_rave.py --rotate none         # 回転なし(既定は右に90度回転)
    dk ./devkit_direct_hdmi_rave.py --no-judge            # 撮影/判定機能を無効化(単純パススルーのみ)
    dk ./devkit_direct_hdmi_rave.py --server-url http://127.0.0.1:8766
    事前にべた褒め版サーバーを起動しておくこと: dk ./fashion_judge_server_rave.py --port 8766
...
DEFAULT_SERVER_URL = "http://127.0.0.1:8766"  # べた褒め版サーバー(fashion_judge_server_rave.py)の既定ポート
DEFAULT_JUDGE_TIMEOUT = 60

# ブランド文字オーバーレイ。`fonts-noto-cjk`(apt)で入る実在のフォントファイル。
OVERLAY_TEXT_DEFAULT = "コーデべた褒めジャッジ"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="USBカメラ映像をDevKit自身のHDMI出力へ直接表示する(べた褒め版)")
    ...
    parser.add_argument("--server-url", default=DEFAULT_SERVER_URL,
                         help=f"fashion_judge_server_rave.pyのURL(既定: {DEFAULT_SERVER_URL}、"
                              "このDevKit自身の上で動いているべた褒め版サーバーを指す)")
...
def render_button_png(path: Path, width: int, height: int, label: str,
    ...
    ブランド文字「コーデべた褒めジャッジ」と向きを揃えるため。保存されるPNGの
...
def judge_via_http(server_url: str, timeout_s: int, image_bytes: bytes) -> dict:
    ...
    try:
        ...
        raise JudgeError(
            ...
            "先に `dk ./fashion_judge_server_rave.py --port ...` 等でサーバーを"
            "起動済みか確認してください。"
        ) from exc
...
class JudgeOverlayController:
    ...
    def _capture_and_send(self) -> None:
        ...
        self._show_message("審査中... べた褒めギャルが褒めポイント探してるので待ってて♥")
