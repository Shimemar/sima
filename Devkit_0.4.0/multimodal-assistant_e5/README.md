...
### Self-Contained Local Models Directory
To keep every downloaded model inside the example directory instead of the shared `/media/nvme/llima/models`, point `LLIMA_MODELS_PATH` at a `models/` subdirectory before running setup:
...
`INSTALL_TTS_VOICES=0` skips re-downloading Piper voices when they are already installed under `src/python/ui/assets/`. `setup.sh` writes `config.local.yaml` with every model path already pointed at `models/`, so `./run.sh` needs no further changes.

To add a second chat/VLM model into the same local `models/` directory (`setup.sh` only installs one), download it directly with the app venv's `hf` CLI and add it to `config.local.yaml` by hand:
...
Then add the entry under `server.models.chat` as shown in [Configure Chat/VLM Models](#configure-chatvlm-models), using the local `models/<name>` path instead of `/media/nvme/llima/models/<name>`.
...
### ASR warmup fails with `key 'language_token_ids' not found`
`src/python/server/main.py` can fail immediately after start with:
...
This means the on-disk `whisper-small-a16w8` model directory has a `whisper_config.json` from an older/partial download that predates the schema the installed `pyneat`/llima runtime expects. It is not caused by `setup.sh` itself — a fresh `hf download simaai/whisper-small-a16w8` (what `setup.sh` runs) does include `language_token_ids`. Fix it by re-downloading the ASR model directory:
...
Because `run.sh` only waits `MODEL_SERVER_START_DELAY` (default 2s) before also starting the Flask UI, this warmup failure can happen *after* the UI has already started — the browser UI loads, but `/v1/chat/completions` and `/v1/audio/transcriptions` are dead. Always confirm `curl -s http://127.0.0.1:9998/v1/models` lists every configured model before trusting the browser UI.

### `port 9998 is already accepting connections`
`src/python/server/main.py` refuses to start a second time on purpose. This example has no lock file, so it cannot tell a genuinely stale process from another terminal/session intentionally running the server. Before killing anything, confirm who owns it:
...
Only stop those PIDs if you are sure nothing else depends on that instance.

### `config.local.yaml` has duplicate top-level keys
If two `setup.sh`/manual-edit operations touch `config.local.yaml` around the same time (e.g. two terminals, or a session on the host and one on the DevKit editing the same NFS-mounted file), the writes can interleave and leave duplicate `app:`/`models:` blocks, sometimes mixing old and new model paths. `yaml.safe_load` silently keeps only the last occurrence of a duplicate key, so this can go unnoticed while leaving stale paths in the file. Regenerate it from scratch rather than hand-patching a suspicious file:

```bash
wc -l config.local.yaml   # sanity check: should have exactly one `server:` and one `app:` block
```

If corrupted, re-run `setup.sh` with the same `LLIMA_MODELS_PATH`/`CONFIG_PATH` used originally (this overwrites the file), then re-apply any manual edits such as extra `server.models.chat` entries.
